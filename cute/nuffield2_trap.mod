...
var dvdx {i in 0..N, j in 0..N} >= 0;
s.t. dvdx_def {i in 0..N-1, j in 0..N}: 
    (dvdx[i,j]+dvdx[i+1,j])/2 = (v[i+1,j]-v[i,j])/h;

##dv/dy 

var dvdy {i in 0..N, j in 0..N} >= 0;
s.t. dvdy_def {i in 0..N, j in 0..N-1}:
    (dvdy[i,j]+dvdy[i,j+1])/2= (v[i,j+1]-v[i,j])/h;

##d2v/dx2

var d2vdx2 {i in 0..N, j in 0..N} >= 0;
s.t. d2vdx2_def {i in 0..N-1, j in 0..N}:
    (d2vdx2[i,j]+d2vdx2[i+1,j])/2 = (dvdx[i+1,j]-dvdx[i,j])/h;

##d2v/dy2

var d2vdy2 {i in 0..N, j in 0..N} >= 0;
s.t. d2vdy2_def {i in 0..N, j in 0..N-1}:
    (d2vdy2[i,j]+d2vdy2[i,j+1])/2 = (dvdy[i,j+1]-dvdy[i,j])/h;

##d2v/dxdy

var d2vdxdy {i in 0..N, j in 0..N};
s.t. d2vdxdy_def {i in 0..N, j in 0..N-1}:
    (d2vdxdy[i,j]+d2vdxdy[i,j+1])/2 = (dvdx[i,j+1]-dvdx[i,j])/h;
...
subject to prob{i in 0..N, j in 0..N}:
	dvdx[i,j] + dvdy[i,j] <= 1;

subject to conv{i in 0..N, j in 0..N}:
	(d2vdy2[i,j])-(d2vdxdy[i,j])^2/d2vdx2[i,j] >= 0; #1.0e-8;

subject to bound: v[0,0]=0;

let {i in 0..N, j in 0..N} d2vdx2[i,j] := 1;
