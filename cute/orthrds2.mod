...
param xd{i in 1..npts} := if (tdp_lo <= i <= tdp_hi) then 1.1 else ((1+tz3^2)+cos(incr*(i-1)))*cos(incr*(i-1))*(1+psize*cos(incr*(i-1)*pseed));
param yd{i in 1..npts} := if (tdp_lo <= i <= tdp_hi) then 1.1 else ((1+tz3^2)+cos(incr*(i-1)))*sin(incr*(i-1))*(1+psize*cos(incr*(i-1)*pseed));
