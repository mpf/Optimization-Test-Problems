...
minimize f: 0 ;
subject to cons{i in 1..M, j in 1..M}:
 (20*y[i,j]-8*y[i-1,j]-8*y[i+1,j]
...
fix {j in -1..M+2} y[-1,j] := 0.0;
fix {j in -1..M+2} y[0,j] := 0.0;
fix {i in 1..M} y[i,-1] := 0.0;
fix {i in 1..M} y[i,0] := 0.0;
fix {i in 1..M} y[i,M+1] := 0.0;
fix {i in 1..M} y[i,M+2] := 0.0;
fix {j in -1..M+2} y[M+1,j] := -H/2;
fix {j in -1..M+2} y[M+2,j] := H/2;
