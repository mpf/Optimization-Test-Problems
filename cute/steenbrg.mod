...
param cmd{1..narcs} := nonz;
param cmr{1..narcs} := nonz;
param la{i in 1..narcs} := if (i==4) then 0.5*cost[i]*alph else cost[i]*alph;
param lt{i in 1..narcs} := cost[i]*tzero;
param lc{i in 1..narcs} := cost[i]*ccr;

param rhs{1..ntrips,1..6} default 0.0;

var cd{i in 1..narcs} >= cmd[i], := 0.1;
var cr{i in 1..narcs} >= cmr[i], := 0.1;
