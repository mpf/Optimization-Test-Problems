...
param xd{i in 1..npts} := ((1+tz3^2)+0.25*cos(incr*(i-1)))*cos(incr*(i-1))*(1+psize*cos(incr*(i-1)*pseed));
param yd{i in 1..npts} := ((1+tz3^2)+0.25*cos(incr*(i-1)))*sin(incr*(i-1))*(1+psize*cos(incr*(i-1)*pseed));
